...
def get_nn(n_neurons, Ndim, reg):
    """ Get NN with specified architecture and regularization """


    num_blocks = 3 # number of layer blocks
    ...
    for bb in range(num_blocks):
        # block bb
        hidden = keras.layers.Dense(n_neurons,
                    input_shape=[Ndim],
                    activation="swish",
        ...
        bus = tf.keras.layers.Add()([bus, block])
        # hiddens.append(hidden)

    # final output
    output = keras.layers.Dense(1, name="prediction",
                                input_shape=[Ndim],
                                activation = 'linear',
                                )(bus)
...
NN_model = get_nn(neuron_val, Ndim, reg)
...
plt.semilogy()
plt.savefig(SAVE_PATH+"loss_history_resnet_get_neuron.png", format="png", dpi=300)
plt.close(fig)
...
plt.savefig(SAVE_PATH+"actual_vs_predicted_resnet_get_neuron.png", format="png", dpi=300, bbox_inches="tight")
plt.close(fig)
